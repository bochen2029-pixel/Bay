...
interface Settings {
  ...
  staleness_days: number;                  // default 14
  ...
  show_done_items: boolean;                // default true; archive (hide) done items when false
...
- **On port conflict**: command returns `PORT_IN_USE`. User can change port in settings (add `lan_capture_port: number` if needed; v1 hardcodes 47821 — flag in §10).
...
- Settings toggle `show_done_items` (default true). When false, done items hidden entirely.
- Done items don't count toward A/B caps.

Demo: mark item blocked with reason; mark another done; done collapses inline; toggle setting to hide done; cap logic unaffected by done count.

### I-09 — Dates + staleness + calendar view

- Strip overflow: Set Start Date / Set Due Date → native date input → `set_item_date` command.
- Staleness: compute `days_since_last_event(item_id)`; if `> settings.staleness_days`, Strip shows ⚠ badge.
- `CalendarView`: month grid (`date-fns`), `DayCell` highlights days with items. Click → day sheet listing items with that start or due.
- View switcher wired.

Demo: set a due date; day shows pill on calendar; leave an A item untouched 14+ days (or set back-dated `created_at` in DB for testing) → ⚠ appears.
...
Demo: move item around, open inspector, see full event trail; enter time-travel at a past timestamp, see old board state; exit.

### I-11 — Settings page + keychain

- `SettingsView` with sections: General (hotkey, staleness days, show done), Capture (LAN toggle — non-functional stub), LLM (base_url, model, api_key input, timeout).
...
Demo: change staleness to 7 days → ⚠ now appears on 7-day-old items; enter LLM API key, verify via `keyring` CLI that it's in keychain.
...
## 10. Decisions needed from Bo

Each item lists the ambiguity, the recommended default (used if you don't respond), and the alternative if you push back.
...
*End of SPEC.md v1.0. Revisions: versioned successors, not in-place edits. Deltas logged in commit messages. Decisions from §10 resolved inline on Bo response, each with a commit note pointing to this section.*
