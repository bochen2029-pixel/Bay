...
- Delete: calls delete_item (soft). Projection excludes; event
  preserved.
- Settings: add show_done_items boolean (default true) with toggle in
  a temporary dev location (Settings page is I-11). When false, done
  items omitted from itemsByTier. (Implement the toggle as a simple
  button in the top bar for now; move to real settings in I-11.)
...
- Toggle show_done_items off → done items disappear from render
- Delete an item → gone from render; sqlite3 shows items.deleted=1,
  event_log has ITEM_DELETED
...
- Staleness: compute days since max(events.ts WHERE item_id = X). If
  > settings.staleness_days (default 14, settable in §I-11), strip
  shows ⚠ badge.
...
- Back-date an item's creation via direct SQL (INSERT INTO events ...
  ts = <15d ago>) or set staleness threshold to 1 day → ⚠ appears
...
Actually do implement the projection rebuild:
- Add commands/events.rs::rebuild_projection command that truncates
  items (preserving deleted rows) and replays all events. Expose in
  Settings later. For I-10, just verify it works via manual command.
...
- SettingsView with sections (see SPEC §5.3 for fields):
  - General: hotkey (configurable; press-to-capture widget), staleness
    days (number input), show_done_items (toggle), close-to-tray
    (toggle, default true per SPEC §10.10)
...
- Remove the temporary show_done_items button from I-08's top bar.
  Routes go through real Settings now.
...
*End of PROMPTS.md v1.0. Revise when SPEC.md changes materially. Per-prompt wording is tuned to resist specific failure modes; change with care.*
