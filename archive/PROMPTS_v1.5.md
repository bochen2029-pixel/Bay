...
> vitest 93/93): I-15 command palette, I-16 C-tier collapse, I-17 undo
> (Ctrl+Z), I-18 audit-log search, I-19 batch ops, I-20 LLM re-org
> proposals. Each follows the same scope/out-of-scope/demo/verify
> structure as I-01..I-14. I-21..I-27 (recurring, streaming; sync,
> multi-profile, theming, plugin surface, mobile companion) remain a
> forward-reference pointing at `FUTURE_WORK.md` — not built, no full
> prompts yet. Co-pass with CLAUDE.md v1.7 → v1.8 and SPEC.md v1.6 →
> v1.7. Prior versions through archive/PROMPTS_v1.4.md.
...
```
Execute SPEC.md §9 "Post-v0.2.0-correctness delivered" I-15 (command
palette).

This is pure UI over commands that already exist. No new write paths,
no new commands, no new event types.

Scope:
- A command-palette overlay opened by Cmd/Ctrl+K (and closed by Esc).
- Fuzzy search across four command kinds:
  - navigate: switch view (Board / Calendar / Time-travel / Archive /
    Settings / Audit log)
  - create: "+ Add to Inbox / A / B / C" (opens the same inline-add /
    swap path as the bay "+ Add"; respects caps exactly as the bay does)
  - jump-to-item: match item content; selecting sets selectedItemId and
    reveals the strip (switching to Board if needed)
  - actions on the selected item: Mark Done / Mark Blocked / Delete /
    Edit — routing through the EXISTING store actions, not new ones
- Keyboard-first: arrow keys move the highlight, Enter runs the
  highlighted command, the list re-ranks as the query narrows.

Out of scope:
- Any new Tauri command or event type (this is a frontend-only feature)
- A swap path different from the existing one (full A/B behaves exactly
  as it does for bay "+ Add")
- Persisting recent commands / palette history

Demo:
- Press Cmd/Ctrl+K → palette opens, autofocused.
- Type a few letters of an item's content → it appears under
  "Jump to" → Enter → board reveals and highlights that strip.
- Type "add a" → "Add to A" appears → Enter → inline add on A (or swap
  modal if A is full).
- Esc closes the palette with no side effects.

Verify:
- No new rows in the commands table (§5.1) and no new event types —
  every palette action reuses an existing store action / command.
- Caps still hold via the palette: "Add to A" when A is full offers the
  same swap modal, never an uncapped insert.
- cargo build / cargo test unaffected (no Rust change); vitest covers
  palette open/close + fuzzy-match ranking.

Stop after I-15.
```

### I-16 — C-tier collapse at >50 items

```
Execute SPEC.md §9 I-16 (C-tier collapse) — the resolution of SPEC
§10.12.

Scope:
- When tier C holds more than 50 items, render only the first 50 (by
  rank) followed by a "Show all (N)" button.
- Clicking "Show all" reveals the remainder for the current session;
  state is session-scoped Zustand UI state — NOT persisted, NOT a
  Settings field.
- Collapse applies to C only. Inbox/A/B are unaffected (A/B are capped;
  Inbox is expected small).
- No virtualization library (no @tanstack/virtual) — this is a plain
  slice + button per §10.12.

Out of scope:
- Virtualization of any tier (explicitly deferred in §10.12)
- Collapse for Inbox / A / B
- Persisting the expanded/collapsed state across launches

Demo:
- Create 51+ C items → C shows the first 50 and a "Show all (51)"
  button.
- Click "Show all" → all 51 render; the button disappears for the
  session.
- Restart the app → C is collapsed again (state is session-scoped).
- With ≤50 C items, no button renders.

Verify:
- The threshold is strictly >50 (50 items shows no button; 51 does).
- Collapsed slice is the first 50 by rank, consistent with the bay's
  normal rank ordering.
- No new dependency in package.json; no Rust change.
...
```
Execute SPEC.md §9 I-17 (undo) — read SPEC §3.6 before writing code.

Undo appends COMPENSATING events over the log. It NEVER updates or
deletes past events (CLAUDE.md §3 — events is append-only; the
migration-002 trigger will ABORT any UPDATE/DELETE anyway).

Scope:
- Ctrl+Z reverses the most-recent user action by appending the inverse
  event(s):
    ITEM_CREATED        → ITEM_DELETED
    ITEM_EDITED         → ITEM_EDITED (content back to content_before)
    ITEM_MOVED          → ITEM_MOVED (tier/rank back to *_before)
    ITEM_STATE_CHANGED  → ITEM_STATE_CHANGED (state back to state_before,
                          restoring the prior blocked_reason — see §4.3)
    ITEM_DATE_SET       → ITEM_DATE_SET (value back to value_before)
    ITEM_DELETED        → ITEM_RESTORED
    ITEM_RESTORED       → ITEM_DELETED
- "Action" granularity: the most-recent run of events sharing the same
  (ts, type). This generalizes undo to batch-undo (I-19): one Ctrl+Z
  reverses an entire batch written under one ts.
- The compensating events go through the normal write_events path
  (atomic, projection-applied), so undo is itself auditable in the log.

Out of scope:
- Redo (not in this increment)
- A transaction-id column to group actions precisely — the (ts, type)
  heuristic is the v1 mechanism; document the known limitation in
  QUESTIONS.md Q01 (coincident same-type actions could be mis-grouped;
  the transaction-id fix is deferred and gates I-21 recurring).
- Undoing LLM suggestion events (the firewall events are not user
  board-actions; leave them out of the undo set)

Demo:
- Move an item A→C, press Ctrl+Z → it returns to A; the log shows the
  original ITEM_MOVED plus a compensating ITEM_MOVED (no rows deleted).
- Block an item with a reason, press Ctrl+Z → it returns to active;
  the compensating event restores cleanly with NO migration-002 CHECK
  abort (this is exactly the P2e fix (a) path).
- Delete an item, Ctrl+Z → ITEM_RESTORED appended, item reappears.

Verify:
- No UPDATE/DELETE on events ever issued (confirm the only new rows are
  appended inverse events).
- Undo of an unblock does not abort on the blocked⇒reason CHECK.
- A batch (I-19) written under one ts is undone by a single Ctrl+Z.

Stop after I-17.
```

### I-18 — Audit-log search

```
Execute SPEC.md §9 I-18 (audit-log search).

Scope:
- search_events command (§5.1): pure-Rust, read-only over the events
  table. Case-insensitive substring match on events.payload, plus
  optional filters: type, item_id, since_ts, until_ts, limit.
- AuditLogView: a top-level view (reachable from the view switcher and
  the command palette) listing matching events newest-first, with the
  query box and the type/item/date filters wired to search_events.
- Each result row is human-formatted (timestamp, type, item content
  where resolvable) and links to the item's Inspector where applicable.

Out of scope:
- Full-text indexing (plain substring is sufficient at single-user
  scale; no FTS5)
- Any write path — this view is strictly read-only over events
- Editing or deleting events from the view (events are append-only)

Demo:
- Open the Audit log view → type a word that appears in some item
  content / reason → matching events list newest-first.
- Add a type filter (e.g. ITEM_MOVED) → list narrows to moves.
- Set a date range → list narrows to that window.

Verify:
- search_events is read-only (no events written by a search).
- Substring match is case-insensitive over payload; filters compose
  (AND semantics).
- Empty query with a type filter returns all events of that type within
  any date bounds.

Stop after I-18.
```

### I-19 — Batch operations

```
Execute SPEC.md §9 I-19 (batch operations) — read SPEC §3.5 before
writing code.

Scope:
- Multi-select on strips: click selects; shift-click extends a range.
  A visible selection state, with a way to clear it (Esc / click-away).
- Two batch commands:
    batch_set_state { ids, state, blocked_reason? } → Item[]
    batch_delete    { ids }                          → void
- Each batch is ONE write_events transaction. NO new event types: each
  selected item emits its own ITEM_STATE_CHANGED / ITEM_DELETED, and
  every event in the batch shares one ts (so undo (I-17) treats the
  whole batch as one action).
- Caps enforced ACROSS the whole batch: when a batch_set_state would
  set items active into A/B, check against the cap using incremental
  projected counters so the batch cannot push a tier over its cap
  (return CAP_EXCEEDED for the whole batch if it would).
- blocked_reason rules unchanged (REASON_REQUIRED when state == blocked).

Out of scope:
- New event types (the whole point is reuse — per-item events under one
  ts)
- A batch move command (this increment is state-change + delete only)
- Partial application (the batch is atomic — all or nothing)

Demo:
- Shift-click a range of Inbox items → "Mark done" → all flip to done
  in one tx; one Ctrl+Z (I-17) reverses the whole batch.
- Select several items spanning more active items than A's free slots →
  attempt a batch activate into A → CAP_EXCEEDED; nothing is applied.
- Select a range → batch delete → all soft-deleted in one tx; events
  share one ts.

Verify:
- One write_events tx per batch (events share a ts); rollback is
  all-or-nothing (the §11 atomicity property covers this).
- Cross-batch cap math: a batch cannot leave A/B over cap.
- No new event types introduced; undo groups the batch by (ts, type).

Stop after I-19.
```

### I-20 — LLM re-org proposals (accept path)

```
Execute SPEC.md §9 I-20 (LLM re-org proposals) — read SPEC §8.7 and
§4.3 (LLM_SUGGESTION_ACCEPTED) before writing code.

The LLM firewall is UNCHANGED. The LLM proposes; the human accepts; the
deterministic tier writes. This is the v2 re-org surface CLAUDE.md §2
always preserved ("Re-org proposals (v2+) must be presented as an
atomic accept/reject diff"), NOT a firewall change. Do not add any LLM
write path, auto-apply, or capture-time suggestion.

Scope:
- System prompt (llm/prompt.rs) evolves from "observe only / do not
  suggest reassignments" to "observe + OPTIONALLY propose a re-org the
  user accepts or rejects," emitting an optional `proposals` array
  alongside `observations` (schema in SPEC §8.2).
- analyze parses and returns `proposals` (action: "move" with to_tier /
  "done" / "active"); the full set is recorded in the
  LLM_SUGGESTION_GENERATED payload.
- AnalyzePanel renders proposals as an accept/reject diff. The user
  selects a subset (none / some / all).
- accept_suggestion { suggestion_event_id, ops? } applies the accepted
  ops in ONE atomic, cap-enforced write_events tx ("move" → ITEM_MOVED;
  "done"/"active" → ITEM_STATE_CHANGED) and writes
  LLM_SUGGESTION_ACCEPTED with resulting_event_ids set to the produced
  event ids (previously ALWAYS empty — see SPEC §4.3 / §10.5). An empty
  ops list is the observations-only acknowledgement (empty array)
  case, unchanged.

Out of scope — DO NOT ADD:
- Any path where the LLM output mutates state without an explicit human
  accept (the firewall holds)
- Auto-tiering, silent re-org, or capture-time tier suggestions (still
  prohibited per SPEC §8 / CLAUDE.md)
- Streaming (I-22) and recurring (I-21)

Demo (requires the I-13 Ollama setup):
- Board with some stale A items and a resolved blocker → Analyze →
  observations AND a proposals diff appear.
- Accept a subset (e.g. one "move A→C" and one "active") → the moves
  apply in one tx; the LLM_SUGGESTION_ACCEPTED event's
  resulting_event_ids lists exactly those produced events.
- Accept nothing / "Mark reviewed" → LLM_SUGGESTION_ACCEPTED with empty
  resulting_event_ids (acknowledgement), no item mutations.

Verify:
- No mutation happens at analyze time — only at accept time, only for
  accepted ops.
- accept_suggestion is atomic and cap-enforced across the accepted
  batch (a re-org cannot push A/B over cap).
- resulting_event_ids matches the ids of the events the accepted ops
  produced; empty ops → empty array.
- The firewall is intact: there is still no code path where LLM output
